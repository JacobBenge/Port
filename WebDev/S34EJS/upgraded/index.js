...
app.get('/cats', (req, res) => {
    ...
    res.render('cats', { cats, title: 'List of Cats' })
})

app.get('/rand', (req, res) => {
    const num = Math.floor(Math.random() * 10) + 1
    // res.render('random', {rand: num}) // use rand as the variable name on .ejs file.
    res.render('random', { num, title: 'Random Generator' }) // or just pass as is
})
...
app.get('/r/:subreddit', (req, res) => {
    ...
    if (data) {
        res.render('subreddit', { ...data, title: subreddit.toUpperCase() });
    } else {
